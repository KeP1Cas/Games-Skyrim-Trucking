...
function App() {
    ...
    for (let cityIndex = 0; cityIndex < newCityStorages.length; cityIndex++) {
      ...
      for (let goodIndex = 0; goodIndex < storage.length; goodIndex++) {
        ...
        const priceChangeValue =
          getRandomInt(goodData.maxSteps) * priceChangeSign;
  ...
  const buyGoods = (goodId, number, price) => {
    ...
    if (money >= totalPrice) {
      ...
      if (index > -1) {
        ...
        if (goodIndex > -1) {
          const newQty = parseInt(storagesNew[index].storage[goodIndex].productList) + parseInt(number, 10)
          storagesNew[index].storage[goodIndex].productList = newQty
        } else {
          storagesNew[index].storage.push({
            id: goodId,
            productList: number
          })
          // Заебись считает! прям найс! исправить....  
        }
      }
